...
export function renderCard(e: Event, landscapeLogos: Record<string, string>): string {
  ...
  const sl = (url: string|undefined, title: string, icon: string) => url ? `<a href="${esc(url)}" target="_blank" rel="noopener noreferrer" class="social-link" title="${title}" aria-label="${title}">${icon}</a>` : '';
